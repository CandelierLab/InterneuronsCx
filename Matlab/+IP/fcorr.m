...
switch in.Results.method
    ...
    case 'parabolic'
        ...
        if u(1)<1 || u(3)>size(res,2)
            dx = size(res,2)/2 + 1 - x0;
        else
        v = sqrt(res(y0, u));
        dx = size(res,2)/2 + 1 + real((u(3)^2*(v(1)-v(2)) + u(2)^2*(v(3)-v(1)) + u(1)^2*(v(2)-v(3)))/...
                                 (u(3)*(v(2)-v(1)) + u(2)*(v(1)-v(3)) + u(1)*(v(3)-v(2)))/2);
        end
                             
        u = y0 + [-1 0 1];
        if u(1)<1 || u(3)>size(res,1)
            dy = size(res,1)/2 + 1 - y0;
        else
            v = sqrt(res(u, x0));
            dy = size(res,1)/2 + 1 + real((u(3)^2*(v(1)-v(2)) + u(2)^2*(v(3)-v(1)) + u(1)^2*(v(2)-v(3)))/...
                (u(3)*(v(2)-v(1)) + u(2)*(v(1)-v(3)) + u(1)*(v(3)-v(2)))/2);
        end
